...
## Studio Session 01: Intro & Full Stack Setup

## Projektidee

Für das Semesterprojekt wurde die Webanwendung **WanderNotes: My Travel Diary** entwickelt. Die Idee ist ein persönliches Reisetagebuch, in dem Nutzer besuchte Orte speichern, Reisen organisieren und ihre Erlebnisse mit Notizen, Bewertungen und Bildern dokumentieren können.

Das Ziel ist eine übersichtliche Anwendung, mit der Erinnerungen an Reisen langfristig festgehalten werden können.

## Erste User Story

> Als Nutzer möchte ich meine gespeicherten Reiseeinträge als Liste sehen, damit ich einen schnellen Überblick über meine bisherigen Reisen bekomme.
...
> Stack: React + Vite (Frontend, Port 5173), Express (Backend, Port 3000). Erstelle eine Webanwendung, die eine Liste von Reiseeinträgen anzeigt.

### Iteration 2

**Prompt:**

> Stack: React + Vite (Frontend, Port 5173), Express (Backend, Port 3000). Erstelle eine Webanwendung, die eine Liste von Reiseeinträgen anzeigt. Jeder Eintrag soll einen Titel, einen Ort und eine kurze Beschreibung besitzen. Die Daten sollen vom Express-Backend geladen und im Frontend dargestellt werden.

## Ergebnis

Das Grundgerüst der Anwendung wurde erfolgreich erstellt. Frontend und Backend konnten lokal gestartet werden und das Frontend zeigte die Daten des Backends korrekt an.
...
## Studio Session 08: Async Messaging
...
## Studio Session 10: Deployment auf Hetzner Webhosting
...
## Studio Session 11: Polish

## Sicherheit

Für die Sicherheitsüberprüfung wurde CodeSniper verwendet. Die wichtigsten kritischen Probleme wurden anschließend behoben.

Wichtige Verbesserungen:

- Helmet für sichere HTTP-Header integriert.
- `X-Powered-By` Header entfernt.
- Rate Limiting für Login und Registrierung hinzugefügt.
- Upload-Limits und MIME-Type-Validierung implementiert.
- Sichere Dateinamen mit `crypto.randomUUID()`.
- Passwort-Hashing mit höherem bcrypt-Cost-Faktor.
- Socket.IO-CORS für die Produktionsumgebung angepasst.
- Schutz gegen Path-Traversal bei Datei-Uploads ergänzt.

Nach diesen Änderungen wurden keine kritischen Sicherheitsprobleme mehr festgestellt.

## Performance (Lighthouse)

Zur Performance-Analyse wurde Lighthouse verwendet. Die wichtigsten Ergebnisse sind in der folgenden Tabelle dargestellt.
...
Die Landing Page wurde durch die Optimierung des Hero-Bildes verbessert. Das Bild wurde verkleinert und mit `next/image` eingebunden, um die Ladezeit zu reduzieren.

Weitere Optimierungen wurden bewusst nicht durchgeführt. Einige Lighthouse-Hinweise beziehen sich auf verwendete Bibliotheken oder Frameworks und hätten größere Änderungen am Projekt erfordert. Da die Anwendung stabil läuft und alle Projektanforderungen erfüllt, wurde auf diese Änderungen verzichtet.

## Landing Page

Vor dem Login wurde eine neue Landing Page erstellt, um die Anwendung übersichtlich zu präsentieren.

Folgende Verbesserungen wurden umgesetzt:

- klare Überschrift mit kurzer Beschreibung der Anwendung
- auffälliger Button zur Registrierung
- Übersicht der wichtigsten Funktionen
- echter Screenshot der Anwendung
- verbesserte Navigation zwischen Landing Page, Login und Registrierung

Dadurch erhalten neue Nutzer bereits vor dem Login einen guten Überblick über die Funktionen der Anwendung.

## Bonus: Automatisches Deployment

Zusätzlich wurde ein automatisches Deployment mit GitHub Actions eingerichtet.

Dafür wurden:

- GitHub Repository Secrets erstellt
- ein GitHub-Workflow erstellt
- das Frontend automatisch gebaut
- die Dateien automatisch auf den Hetzner-Server übertragen

Nach jedem Push auf den `main`-Branch wird das Frontend automatisch gebaut und auf den Server übertragen. Das Deployment wurde erfolgreich getestet und die Anwendung funktioniert weiterhin fehlerfrei.
...
Während des Projekts wurden viele Architekturentscheidungen getroffen, die sich im Nachhinein als sinnvoll erwiesen.

Rückblickend würden wir jedoch einige Dinge früher umsetzen:

- Service Layer bereits zu Beginn konsequent verwenden.
- Deployment früher testen.
- Tests parallel zur Entwicklung schreiben.
- Die Landing Page bereits früher integrieren.
